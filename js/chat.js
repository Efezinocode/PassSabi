...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function buildLessonPrompt(action, answerText) {
    const session = getCurrentSession();
    return buildLessonPromptFromStudy(action, answerText, session);
  }

  function buildLessonPromptFromStudy(action, answerText, session) {
  ...
  async function handleLessonToolAction(action, context = {}) {
    ...
    const lesson = buildLessonPromptFromStudy(
      action,
      context.answerText || "",
      getCurrentSession()
    );
